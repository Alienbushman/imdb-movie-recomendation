# Progress — Ticket 009

Rows are ordered by execution order. Work top-to-bottom unless dependencies allow otherwise.

| ID | Title | Priority | Status | Dependencies | Notes |
|---|---|---|---|---|---|
| ST-001 | Backend: Title Search Endpoint | High | Done | — | Autocomplete for seed title selection |
| ST-002 | Backend: Similarity Engine + API Endpoint | High | Open | ST-001 | Core similarity scoring + API |
| ST-003 | Frontend: Similar Page with Search + Routing | Medium | Open | ST-001 | New page, nav, autocomplete |
| ST-004 | Frontend: Results Display + Seen/Unseen Filter | Medium | Open | ST-002, ST-003 | Cards, filters, sort |

## Completion

1 / 4 complete
